...
const CusisinesFilter = ({onChange, selectedCuisines, isExpanded, onExpandedClick}: Props) => {
    ...
    <div className="flex justify-between items-center px-2">
        ...
        <div className="space-y-2 flex flex-col">
            ...
            <Button onClick={onExpandedClick} variant="link" className="mt-4 flex-1">{isExpanded ? (<span className="flex flex-row items-center">View Less <ChevronUp/></span>):(<span className="flex flex-row items-center">View More <ChevronDown/></span>)}</Button>
        </div>
    </>
  )
}

export default CusisinesFilter; //orginal