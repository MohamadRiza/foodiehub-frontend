...
const MenuInputItem = ({ index, removeMenuItem }: Props) => {
  const { control } = useFormContext();

  return(
    <div className="flex flex-row items-end gap-2">
        <FormField control={control} name={`menueItems.${index}.name`} render={({field}) => <FormItem>
        ...
        <FormField control={control} name={`menueItems.${index}.price`} render={({field}) => <FormItem>
